...
var (
	...
	MsgConnectorFailInvoke         = FFE("FF00155", "Connector failed request. requestId=%s failed to call connector API")
	MsgConnectorInvalidContentType = FFE("FF00156", "Connector failed request. requestId=%s invalid response content type: %s")
	MsgConnectorError              = FFE("FF00157", "Connector failed request. requestId=%s reason=%s error: %s")
)
